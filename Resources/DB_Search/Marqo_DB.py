...
def search_db(collection_name, bot_name, user_id, expanded_input, memory_type, selected_domain, search_number):
    ...
    if memory_type != None:    
        ...
        try:
            response = client.index(f"BOT_NAME_{bot_name}").search(
                ...
                limit=18,
                filter_string=f"user:{user_id}",
                # <task> Add memory type as filter
                search_method="TENSOR"
            )
            vector_results = response.get('hits', [])
            all_db_search_results.extend(vector_results)
                ...
                for hit in hits:
                    ...
                    try:
                        response = client.index(f"BOT_NAME_{bot_name}").search(
                            q=expanded_input,
                            limit=5,
                            # <task> Add memory type and domain
                            filter_string=f"user:{user_id}",
                            search_method="TENSOR"
                        )
                        vector_results = response.get('hits', [])
    ...
    if memory_type == "Episodic":
        if all_db_search_results:
            try:
                sorted_results = sorted(all_db_search_results, key=lambda x: x['timestring'])
                top_30_results = sorted_results[:search_number]
                sorted_results = [f"{hit['message']}" for hit in top_30_results]
                return sorted_results
            except Exception as e:
        ...
        if all_db_search_results:
            try:
                sorted_results = sorted(all_db_search_results, key=lambda x: x['_score'], reverse=True)
                top_30_results = sorted_results[-search_number:]
                sorted_results = [f"{hit['message']}" for hit in top_30_results]
                return sorted_results
            except Exception as e:
