...
def upload_memory(collection_name, memory_type, bot_name, user_id, text, domain):
    timestamp = timestamp_func()
    timestring = timestamp_to_datetime(timestamp)
    ...
    if domain != "Conversation History":            
        vector1 = embeddings(model, domain)
        try:
            hits = client.search(
                collection_name=f"BOT_NAME_{bot_name}_Domains",
...
def upload_webscrape(collection_name, bot_name, user_id, text, name, url, task_counter):
    timestamp = timestamp_func()
    timestring = timestamp_to_datetime(timestamp)
    vector = embeddings(model, name + '\n' + text)
    unique_id = str(uuid4())
